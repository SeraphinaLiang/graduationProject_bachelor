<template>
  <v-container style="margin-top: 10px">
    <h3 style="color: #00695c;margin: 10px">
      编辑帖子
    </h3>
    <div>
  ...
  export default {
    ...
    methods: {
      initData () {
        ...
        const _this = this
        this.$axios.get('/post/query?postID=' + postID).then(res => {
          console.log(res.data.data)
          _this.post = res.data.data
          _this.postContent.title = _this.post.title
          _this.postContent.briefing = _this.post.briefing
          _this.postContent.detail = _this.post.detail
        })
      },
            ...
            if (this.postContent.detail !== '') {
              if ((this.postContent.chosenTags.length !== 0) && (this.postContent.select.length !== 0)) {
                this.$axios.post('/post/submit', JSON.stringify(this.postContent), {
                  ...
                  _this.deleteOriginalPost() // 把原来的帖子删掉
      ...
      cancel () {
        ...
        this.postContent.select = []
        this.postContent.chosenTags = []
        this.$router.push('/user')
        this.post = {}
      },
      deleteOriginalPost () {
        const _this = this
        this.$axios.get('/user/delete/post?postID=' + this.post.id, {
          headers: {
            Authorization: localStorage.getItem('token'),
          },
        }).then(res => {
